% 1; %uncomment out if using in script-debug mode
function varargout = vs_trim2nc(vsfile,varargin) %%comment out if using in script-debug mode
	...
	% uncomment out if using in script-debug mode
	% vsfile = "/basin/rmt/okhotsk/scratch/connectivity/nefis/TanonStrait_1.5KMgrid_jan232017/trim-TANON_23JAN2017_2015_TIDEWIND_6.dat"
	% vsfile = "/basin/rmt/okhotsk/scratch/connectivity/nefis/DAVAO_Delft_outputs/2010/trim-DAVAO_2010_1of4.dat"
	% varargin = {'Format', '64bit', 'epsg', 32651, 'timezone', '+08:00'} %
	...
	% addpath('/basin/installers/delft3d-5740/delft3d-5740/src/tools_lgpl/matlab/quickplot/progsrc/');
	...
	% Note: You can make an ncdump cdl ascii file a char for keyword dump:
	...
	%  $Id: vs_trim2nc.m 11967 2015-06-08 21:07:10Z jagers $
	...
	if any(strcmp('YZ',    OPT.var)) | any(strcmp('YWAT',OPT.var))
		OPT.y 				= 1; end
	if any(strcmp('XCOR',  OPT.var));
		OPT.grid_x 			= 1; end
	if any(strcmp('YCOR',  OPT.var));
		OPT.grid_y			= 1; end
	if any(strcmp('DP',    OPT.var)) | any(strcmp('DP0',OPT.var)) | any(strcmp('DPS',OPT.var))
		OPT.grid_depth 		= 1; end
	if any(strcmp('DPS0',  OPT.var));
		OPT.depth 			= 1; end
	if any(strcmp('KCS',   OPT.var));
		OPT.zactive			= 1; end
	if any(strcmp('S1',    OPT.var));
		OPT.waterlevel 		= 1; end
	if any(strcmp('U1',    OPT.var)) | any(strcmp('V1',OPT.var))
		OPT.velocity       	= 1; end
	if any(strcmp('W',     OPT.var));
		OPT.velocity_omega 	= 1; end
	if any(strcmp('WPHY'  ,OPT.var));
		OPT.velocity_z    	= 1; end
	if any(strcmp('TAUKSI',OPT.var)) | any(strcmp('TAUETA',OPT.var))
		OPT.tau            	= 1; end
	if any(strcmp('RTUR1', OPT.var));
		OPT.eps            	= 1;
		OPT.tke            	= 1; end
	if any(strcmp('R1',    OPT.var));
		OPT.temperature    	= 1;
		OPT.salinity       	= 1;
		OPT.sediment       	= 1;
		OPT.tracer         	= 1; end
	if any(strcmp('RHO',   OPT.var));
		OPT.density        	= 1; end
	if any(strcmp('VICWW', OPT.var));
		OPT.viscosity_z    	= 1; end
	if any(strcmp('DICWW', OPT.var));
		OPT.diffusivity_z  	= 1; end
	if any(strcmp('RICH',  OPT.var));
		OPT.Ri             	= 1; end
	...
	% T.datenum = T.datenum(1:1000);  %temporary edit to check if 64bit limit is really being reached
	if OPT.time==0
	...
	%uncomment out if using in script-debug mode
	% break;  %use if netcdf creation needs to be skipped

	%% 1a Create file (add all NEFIS 'map-version' group info)

	disp('1a Create file - add all NEFIS map-version group info');
	...
	nc.Attributes(end+1) = struct('Name','history'            ,'Value', ['Original filename: ',filenameext(vsfile),...
										', ' ,M.version,...
										', file date:',M.datestr,...
										', transformation to netCDF: $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/delft3d/vs_trim2nc.m $ $Id: vs_trim2nc.m 11967 2015-06-08 21:07:10Z jagers $']);
	...
	%check if at limits of netcdf_64bit datatype
	if( length(T.datenum)*G.mmax*G.nmax*G.kmax*4 > 4e+9 )
		filesplits_max = ceil(length(T.datenum)*G.mmax*G.nmax*G.kmax*4 / 4e+9);
	
		opttime_orig = OPT.time;
		filesplits_interval = linspace( 1,length(opttime_orig),filesplits_max+1 );

		tdatenum_orig = T.datenum;
		
		ncfile_orig = ncfile;

		warning("Current nefis file is too large, reducing size by splitting output into %d", filesplits_max)
	end
	
	for filesplits = 1:3
	
		ncfile = sprintf( "%s_%dof%d.nc", ncfile_orig(1:end-3), filesplits, filesplits_max)
		OPT.time = opttime_orig(filesplits_interval(filesplits) : filesplits_interval(filesplits+1) );
		T.datenum = tdatenum_orig(filesplits_interval(filesplits) : filesplits_interval(filesplits+1) );

		length(T.datenum)*G.mmax*G.nmax*G.kmax*4
		...
		disp('2 Create dimension combinations');
		...
		% 2D flipped dimensions due to reversed hycom netcdf standards

		mn.dims(1) = struct('Name', 'm'               ,'Length',ncdimlen.m);
		mn.dims(2) = struct('Name', 'n'               ,'Length',ncdimlen.n);
		...
		% 4D flipped dimensions due to reversed hycom netcdf standards

		mnkt.dims(1) = struct('Name', 'm'               ,'Length',ncdimlen.m);
		mnkt.dims(2) = struct('Name', 'n'               ,'Length',ncdimlen.n);
		mnkt.dims(3) = struct('Name', 'Layer'           ,'Length',ncdimlen.Layer);
		mnkt.dims(4) = struct('Name', 'time'            ,'Length',ncdimlen.time);
		...
		if (~isempty(OPT.epsg)) | (~any(strfind(G.coordinates,'CART'))) % CARTESIAN, CARTHESIAN (old bug)
			if any(strcmp('longitude',OPT.var))
				...
				nc.Variables(ifld) = struct('Name'       , 'longitude', ...
											'Datatype'   , OPT.type, ...
											% 'Dimensions' , nm.dims, ... 
											'Dimensions' , mn.dims, ... % 2D flipped dimensions due to reversed hycom netcdf standards
											'Attributes' , attr,...
											'FillValue'  , []); % this doesn't do anything
			end
		...
		disp('3 Create (primary) variables: momentum and mass conservation');
		...
		if any(strcmp('velocity',OPT.var))
			if ~isequal(vs_get_elm_size(F,'U1'),0) || ~isequal(vs_get_elm_size(F,'V1'),0)
				...
				nc.Variables(ifld) = struct('Name'       , 'velocity_x', ...
											...
											'Dimensions' , mnkt.dims, ... % 4D flipped dimensions due to reversed hycom netcdf standards
											'Attributes' , attr,...
											'FillValue'  , []); % this doesn't do anything
				ifld     = ifld + 1;clear attr dims
				if (~any(strfind(G.coordinates,'CART'))) % CARTESIAN, CARTHESIAN (old bug)
		...
		disp( '3 Create variables: scalars' );
		...
		i = 0;  %% Next to edit to work with different time slices
		...
		if ~(OPT.empty)
			for it = OPT.time % it is index in NEFIS file
				...
				if any(strcmp('velocity',OPT.var))
					...
					D.u      = apply_mask(permute(D.u,[3 2 4 1]),G.cen.mask'); % y x z
					D.v      = apply_mask(permute(D.v,[3 2 4 1]),G.cen.mask'); % y x z
				...
				if any(strcmp('velocity_z',OPT.var))
					...
					matrix      = apply_mask(vs_let_scalar_hycom(F, 'map-series',{it},'WPHY'     , {0,0,0},'quiet'),G.cen.mask'); %% 4D flipped dimensions due to reversed hycom netcdf standards
				...
				if any(strcmp('salinity',OPT.var))
					if isfield(I,'salinity')
						...
						matrix    = apply_mask(vs_let_scalar_hycom(F,'map-series' ,{it},'R1'       , {0 0 0 I.salinity.index   },'quiet'),G.cen.mask'); %% 4D flipped dimensions due to reversed hycom netcdf standards
				...
				if any(strcmp('temperature',OPT.var))
					if isfield(I,'temperature')
						...
						matrix = apply_mask(vs_let_scalar_hycom(F,'map-series' ,{it},'R1'       , {0 0 0 I.temperature.index},'quiet'),G.cen.mask'); %% 4D flipped dimensions due to reversed hycom netcdf standards
				...
				if (any(strcmp('density',OPT.var)) | any(strcmp('pea',OPT.var))) & ~isequal(vs_get_elm_size(F,'RHO'),0)
					...
					matrix = apply_mask(vs_let_scalar_hycom(F,'map-series' ,{it},'RHO'      , {0 0 0},'quiet'),G.cen.mask'); %% 4D flipped dimensions due to reversed hycom netcdf standards
					if any(strcmp('density',OPT.var))
					...
					if strmatch('SIGMA-MODEL', G.layer_model)
						if any(strcmp('pea',OPT.var))
							...
							for k_count = 1:G.kmax+1
								G.cen.intf.z(:,:,k_count) = G.sigma_intf(k_count).*(G.cen.zwl + G.cen.dep) - G.cen.dep; % dep is positive down
							end
							if OPT.debug
		...
		if isnumeric(OPT.dump) && OPT.dump==1
			% ncdump(ncfile);
			system( sprintf( 'ncdump -h %s', ncfile) );
		% else
...
end %%comment out if using in script-debug mode
