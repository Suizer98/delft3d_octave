...
function result = recurse(data, level, addit)
    if iscell(data) && ~yml.tools.ismymatrix(data)
...
function result = iter_cell(data, level, addit)
    ...
    for i = 1:length(data)
        if isstruct(result{i}) && isfield(result{i}, yml.tools.kwd_parent())
            result{i} = inherit(result{i}, result{i}.(yml.tools.kwd_parent()), [], addit{1}, {}); % !!!
        end;
    end;
end
...
function result = iter_struct(data, level, addit)
    ...
    for i = fields(result)'
        fld = char(i);
        if isstruct(result.(fld)) && isfield(result.(fld), yml.tools.kwd_parent())
            result.(fld) = inherit(result.(fld), result.(fld).(yml.tools.kwd_parent()), [], addit{1}, {});
        end;
    end;
end
...
function result = inherit(child, parent_chr, container, oaroot, loc_imported)
    ...
    for i = length(parent_chr):-1:1
        ...
        if isstruct(parentstruct) && isfield(parentstruct, yml.tools.kwd_parent())
            next_loc_imported = loc_imported;
            next_loc_imported{end + 1} = parent_chr{i};
            result = yml.tools.merge_struct(inherit(parentstruct, parentstruct.(yml.tools.kwd_parent()), [], oaroot, next_loc_imported), result, {'import'});
        end;
        result = yml.tools.merge_struct(parentstruct, result, {'import'});
    end;
end
