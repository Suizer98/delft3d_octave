...
%    status = hdfsave_matrix2sds(file_id,FILE_NAME,vgroup_id,SDS_NAME,dat, attr, dims)
%
% where:
% file_id   - 
% FILE_NAME - 
% vgroup_id - 
% SDS_NAME  - SDS name for dat
% dat       - numeric data array; 
% attrs     - cell array of {name, value} attributes (optional)
%             or structure that is internally transformed to an cell array.
% dims      - cell array of dimension names (optional)
%
% Note: do not use HDFSAVE_MATRIX2SDS directly, use through HDFVSAVE.
%
% Calls mat2sdsid(...) to create the SDS. So use the reciprocal function
% sdsid2mat(..) to read the SDS. (To avoid flipping of dimensions).
%
% Follows recipe at:
% <http://hdf.ncsa.uiuc.edu/UG41r3_html/Vgroups.fm4.html#50292>
%
...
% Nov 13 2006: added option so pass a struct as attributes, rather than a cell array.
% Nov 14 2006: error handling for dimension with zero length.
% Jul 25 2007: changed this from error to warning and write one space.
...
% - creates the HDF file FILE_NAME,
% - an SDS in the SD interface, named SDS_NAME, 
% - and a vgroup in the Vgroup interfac, named VG_NAME 
%   and of class VG_CLASS.
% The program then adds the SDS to the vgroup using Vaddtagref/vfadtr. 
% Notice that, when the operations are complete, 
% the program explicitly terminates access to the SDS, 
% the vgroup, the SD interface, and the Vgroup interface 
% before closing the HDF file. 
% Refer to Chapter 3, Scientific Data Sets (SD API) for the discussion of 
% the SD routines used in this example. 
   ...
   opt.debug = 0;
      ...
      %%  4 - Create the SDS
         ...
         if nargin > 5
            attr = varargin{1};
            if isstruct(attr)
               attr = struct2celllist(attr);
            end
            if nargin > 6
               dims = varargin{2};
               sds_id = mat2sdsid(sd_id, SDS_NAME, dat, attr, dims);
            else
               sds_id = mat2sdsid(sd_id, SDS_NAME, dat, attr);
            end
         else
             if any(size(dat)==0)
                disp(['Warning: No dimensions of zero length allowed in ''',SDS_NAME,''', space inserted.'])
                sds_id = mat2sdsid(sd_id, SDS_NAME, ' ');
             else
                sds_id = mat2sdsid(sd_id, SDS_NAME, dat);
             end
         end
