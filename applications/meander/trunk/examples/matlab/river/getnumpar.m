...
numpar.ds = 0.033;                   % Optimal distance between neighbouring points

numpar.sweCFL = 0.04;               % CFL number
numpar.sweacc = 1e-7;              % accuracy cutoff for SWE calculation

numpar.asRCFL = 0.2; 
numpar.asRacc = 5e-9;              % accuracy cutoff for alpha_s/R calculation
...
numpar.smoothrad = 2;              % 0 = adaptation by Johannesson & Parker (1989)
...
numpar.bedtype = 3;                % 0 = flat bed   (A=0)
                                   % 1 = river bed  (A=3)
                                   % 2 = A/R from m89 experiment bed level)
                                   % 3 = A/R from input
