function ITHK_ind_recreation_dunearea(sens)
% function ITHK_ind_recreation_dunearea(sens)
%
% Computes the indicator for the recreation in the dunes, using dune area as a proxy
%
% INPUT:
...
% OUTPUT:
%      S      structure with ITHK data (global variable that is automatically used)
%              .PP(sens).UBmapping.recreation.dunearea
%              .PP(sens).GEmapping.recreation.dunearea
%              .PP(sens).output.kml_recreation_dunearea
...
% $Id: ITHK_ind_recreation_dunearea.m 7463 2012-10-12 08:25:14Z boer_we $
% $Date: 2012-10-12 16:25:14 +0800 (Fri, 12 Oct 2012) $
% $Author: boer_we $
% $Revision: 7463 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ITHK/Matlab/postprocessing/indicators/recreation/ITHK_ind_recreation_dunearea.m $
% $Keywords: $
...
if S.userinput.indicators.recreation == 1
    fprintf('ITHK postprocessing : Indicator for the recreation in the dunes, using dune area change as a proxy\n');
    ...
    Ythr                     = str2double(S.settings.indicators.recreation.dunearea.Ythr);
    sRough                   = S.PP(sens).settings.sgridRough;
    dS                       = S.PP(sens).settings.dsRough;
    zonefile                 = which('ITHK_ind_recreation_dunearea.txt');  % loads a list [Nx2] with center position of the drinkingwater zone (column 1) and the width of the zone (column 2)
    ...
    duneareaclasses          = ones(size(dunearea));
    duneareaclasses(dunearea<Ythr)                      = 2;
    duneareaclasses(dunearea>=Ythr & dunearea<2*Ythr)   = 3;
    duneareaclasses(dunearea>=2*Ythr)                   = 4;
    duneareaclasses(ID_outside,:)                       = 1;
    dunearea(ID_outside,:)                              = 0;
    S.PP(sens).GEmapping.recreation.dunearea  = dunearea;
    S.PP(sens).GEmapping.recreation.dunearea2 = duneareaclasses;
    ...
    PLOTicons    = S.settings.indicators.recreation.dunearea.icons;
    if isfield(S,'weburl')
        for kk=1:length(PLOTicons)
            PLOTicons(kk).url = [S.weburl '/img/hk/' strtrim(PLOTicons(kk).url)];
        end
    end    
    colour       = {[0 0.6 0.0],[0.95 0.0 0.0]};
    fillalpha    = 0.7;
    ...
    S.PP(sens).output.kmlfiles = [S.PP(sens).output.kmlfiles,'S.PP(sens).output.kml_recreation_dunearea2'];
end
end
