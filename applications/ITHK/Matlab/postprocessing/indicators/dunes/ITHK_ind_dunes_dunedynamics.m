function ITHK_ind_dunes_dunedynamics(sens)
% function ITHK_ind_dunes_dunedynamics(sens)
%
...
% OUTPUT:
%      S      structure with ITHK data (global variable that is automatically used)
%              .PP(sens).UBmapping.dunes.dunedynamics
%              .PP(sens).GEmapping.dunes.dunedynamics
%              .PP(sens).output.kml_dunes_dunedynamics
...
% $Id: ITHK_ind_dunes_dunedynamics.m 7463 2012-10-12 08:25:14Z boer_we $
% $Date: 2012-10-12 16:25:14 +0800 (Fri, 12 Oct 2012) $
% $Author: boer_we $
% $Revision: 7463 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ITHK/Matlab/postprocessing/indicators/dunes/ITHK_ind_dunes_dunedynamics.m $
% $Keywords: $
...
if S.userinput.indicators.dunes == 1
    ...
    fprintf('ITHK postprocessing : Indicator for the dynamics of the dunes\n');
    ...
    PLOTicons    = S.settings.indicators.dunes.dunedynamics.icons;
    if isfield(S,'weburl')
    ...
    S.PP(sens).output.kmlfiles = [S.PP(sens).output.kmlfiles,'S.PP(sens).output.kml_dunes_dunedynamics2'];
end
end
