...
% $Id: ITHK_ind_residential_groundwater.m 7463 2012-10-12 08:25:14Z boer_we $
% $Date: 2012-10-12 16:25:14 +0800 (Fri, 12 Oct 2012) $
% $Author: boer_we $
% $Revision: 7463 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ITHK/Matlab/postprocessing/indicators/residential/ITHK_ind_residential_groundwater.m $
% $Keywords: $
...
if S.userinput.indicators.residency == 1
    ...
    zonefile                 = which('ITHK_ind_residential_groundwater.txt');  % loads a list [Nx2] with center position of the drinkingwater zone (column 1) and the width of the zone (column 2)
    ...
    S.PP(sens).output.kmlfiles = [S.PP(sens).output.kmlfiles,'S.PP(sens).output.kml_residential_groundwater2'];
end
end
