function varargout=ITHK_landboundary(cmd,varargin)
%ITHK_LANDBOUNDARY Read/write land boundary files.
%   XY = ITHK_LANDBOUNDARY('read',FILENAME) reads the specified file and returns
%   the data as one Nx2 array.
%
%   [X,Y] = ITHK_LANDBOUNDARY(...) returns separate X and Y arrays.
%
%   ITHK_LANDBOUNDARY('write',FILENAME,XY) writes a landboundary to file. XY
%   should either be a Nx2 array containing NaN separated line segments or
%   a cell array containing one line segment per cell.
%
%   ITHK_LANDBOUNDARY('write',FILENAME,X,Y) writes a landboundary to file. X
...
%
%   ITHK_LANDBOUNDARY(...,'-1') does not write line segments of length 1.
%
%   ITHK_LANDBOUNDARY(...,'dosplit') saves line segments as separate TEKAL
...
%
%   FILE = ITHK_LANDBOUNDARY('write',...) returns a file info structure for the
...
%   $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ITHK/Matlab/preprocessing/operations/ITHK_landboundary.m $
%   $Id: ITHK_landboundary.m 6519 2012-06-21 11:53:36Z huism_b $
...
try
    ...
    for i=1:length(T.Field)
        Data(offset+(1:T.Field(i).Size(1)),:)=ITHK_tekal('read',T,i);
...
TklFileInfo=ITHK_tekal('write',filename,T);
