function [SOSdata2,idNEAREST,idRANGE] = addUNIFORMLYDISTRIBUTEDnourishment(MDAdata,nourishment,SOSfilename)
...
for ii=1:length(nourishment.x)
    ...
    distance        = distXY(MDAdata.QpointsX(idRANGE),MDAdata.QpointsY(idRANGE));
    gridcellwidth   = [0; diff(distance)];
    %gridcellwidth   = [gridcellwidth(1:3:end-2)+gridcellwidth(2:3:end-1)+gridcellwidth(3:3:end)];%every 3 cells
    %gridcellwidth2  = gridcellwidth(idRANGE);
%     if length(idRANGE)>length(distance)
%         spreadingwidth  = abs(distance(idRANGE(1)+1:idRANGE(end)-1)-distance(idNEAREST));
%     else
%         spreadingwidth  = abs(distance(idRANGE(1)+1:idRANGE(end)+1)-distance(idNEAREST+1));
%     end
    %idRANGE2         =[2:3:length(MDAdata.QpointsX(idRANGE))-1];
...
%     volumepercell  = distrfactor2*volumepernourishment;
    ...
    volumepercell      = distrfactor*nourishment.volume;
