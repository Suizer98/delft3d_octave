function LDB = ITHK_io_readldb(filename,varargin)
% READLDB reads landboundary files in nan-separated
%  list struct.
%
%    ITHK_io_readldb reads all lines from a *.ldb landboudnary file
%    into a nan separated list struct (vector map).
%
%    LDB = ITHK_io_readldb(filename) returns the x and y
...
%
%    H =ITHK_io_readldb(filename,illegalmarkers) 
%    sets all values in array illegalmarkers tot nan.
%
%    H =ITHK_io_readldb(filename,illegalmarkers,scale) 
%    DIVIDES x and y with scale.
%
%    H =ITHK_io_readldb(filename,illegalmarkers,[xscale yscale]) 
