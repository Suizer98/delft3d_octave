function zm=optiComputeMean(this,dataGroup,weights)

% OPTICOMPUTEMEAN - computes weighted average from optiStruct (target)
%
% Usage:
%  zm=optiComputeMean(optiStruct,dataGroup,weights(optional));
%
...
zm=nansum(this.input(dataGroup).data.*repmat(weights,size(this.input(dataGroup).data,1),1),2);
