...
netcdf.putAtt(ncid,globalID, 'title',           'SFINCS netcdf bnd/bzs/bzi water level time-series input');
...
% Standard names - 6 = bzi
bzi_ID      = netcdf.defVar(ncid,'zi','double',[pointsdimid timedimid]); 
netcdf.putAtt(ncid,bzi_ID,'standard_name','water_level');
netcdf.putAtt(ncid,bzi_ID,'long_name','incoming_wave_water_level_elevation');
netcdf.putAtt(ncid,bzi_ID,'units','m');
netcdf.putAtt(ncid,bzi_ID,'_FillValue',-999);
