...
% $Id: getVolume.m 12800 2016-07-06 06:21:37Z nederhof $
% $Date: 2016-07-06 14:21:37 +0800 (Wed, 06 Jul 2016) $
% $Author: nederhof $
% $Revision: 12800 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/DUROS/analysis/Volume/getVolume.m $
% $Keywords: $
...
if ~SameProfile
    ...
    %% No difference (within precision) between the two profiles
    volume  = 0;
    x       = min(OPT.x);
...
if ~isempty(Zlimits)
    ...
    [result.xLand, result.zLand, result.xActive, result.zActive, result.z2Active, result.xSea, result.zSea] = deal(OPT.x(idLand), OPT.z(idLand), [], Zlimits, Zlimits, OPT.x(idSea), OPT.z(idSea));
end
