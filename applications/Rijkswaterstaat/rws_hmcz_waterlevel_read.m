function DAT = rws_hmcz_waterlevel_read(varargin)
%RWS_HMCZ_WATERLEVEL_READ   Reads ASCII waterlevel file from www.hmcz.nl
%
%      D = rws_hmcz_waterlevel_read('fname')
%
%   eeads ASCII wwaterlevel data file from www.hmcz.nl of type 
%   WTr2 (see 2nd line) that looks like:
%
%   ----------------------------------------------------------
...
%   ----------------------------------------------------------
%
%   HMCZ_WATERLEVEL_READ returns a struct D containing the data from the 
%   file 'fname' which includes the following fields:
%
%   1 WS10     10 min. gemiddelde waterhoogte in [cm t.o.v. NAP]
%   
...
% $Id: rws_hmcz_waterlevel_read.m 8252 2013-03-01 11:22:45Z m.eelkema.x $
% $Date: 2013-03-01 19:22:45 +0800 (Fri, 01 Mar 2013) $
% $Author: m.eelkema.x $
% $Revision: 8252 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/Rijkswaterstaat/rws_hmcz_waterlevel_read.m $
...
no_cellstr    = 1;
   ...
   if mod(nargin,2)     == 0 
     [shortfilename, pathname, filterindex] = uigetfile( ...
        {'*.src' ,'HMCZ waterlevel file (*.data)'; ...
         '*.*'   ,'All Files (*.*)'}, ...
         'HMCZ waterlevel file (*.data)');
      ...
      elseif fid > 2
         ...
         %% Read 1st block header
           ...
           [dtmn     ,rest] = str2num(strtok(rest));
            ...
            DAT.data(nblock).WL     = repmat(nan,[1 nt]);
            nrow                      = 0;
            ...
            while 1
               ...
               DAT.data(nblock).WL    (nrow) = nan;
               else
               DAT.data(nblock).WL    (nrow) = str2num(tok03); % [m/s]
               end
...
%                end
            ...
            DAT.legend.WL     = '10-min. gemiddelde waterstand [cm]';
...
%         end % try
   ...
   DAT.iomethod = ['© hmcz_waterlevel_read.m  ',mfile_version]; 
