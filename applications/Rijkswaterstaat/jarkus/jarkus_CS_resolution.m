function void = jarkus_CS_resolution(b,e,j,coor)
% jarkus_CS_resolution plots the cross-shore resolution: cross-shore
...
%
%   Example: 
%     jarkus_CS_resolution(7000150,9011850,1990,'RSP')
%      (plots the CS resolution for the Holland coast with the RSP-coordinates)
%     jarkus_CS_resolution(7000150,9011850,1990,'RD')
...
for i=1:length(z(:,1))
    ...
    if ~isempty(z2)
        dx=diff(x);
        xdx=x(1:length(x)-1)+dx./2;
        S(:,i)=interp1(xdx,dx,xi,'linear');  
...
if strcmp(coor,'RSP')
    ...
    title(['Cross-shore resolution (distance between measurements) in m in ' num2str(j)])
else
