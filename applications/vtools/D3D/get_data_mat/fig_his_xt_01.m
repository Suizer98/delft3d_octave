...
%$Author: chavarri $
%$Id: fig_his_xt_01.m 18336 2022-08-26 09:21:36Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/get_data_mat/fig_his_xt_01.m $
%
%MATLAB BUGS:
...
% time_line=datetime([2018,2018,2018,2018,2018,2018,2018,2018],[08,08,09,09,10,10,11,11],[5,17,5,19,12,20,01,23]);
% time_line.TimeZone='+01:00';
% ntl=numel(time_line);
...
%     tim=t_mea{kx};
%     if ~isempty(tim)
%         tim_g=t_mea{kx}(1):days(step_p):t_mea{kx}(end);
%         nv=numel(tim_g);
%         idx_p=NaN(1,nv);
%         for kv=1:nv
%             [~,idx_p(1,kv)]=min(abs(tim_g(kv)-tim));
%         end    
%         han.m=scatter3(d_mea(kx).*ones(1,nv).*s_fact,t_mea{kx}(idx_p),max(val_m_mea(:)).*ones(1,nv),10,val_mea{kx}(idx_p),'s','filled','parent',han.sfig(kr,kc),'markeredgecolor','k');
%     end
% end
% for ktl=1:ntl
%     plot3(lim_dist,repmat(time_line(ktl),1,2),repmat(max(val_m(:)),1,2),'parent',han.sfig(kr,kc),'color','w','linestyle','--','linewidth',2);
...
% han.sfig(kr,kc).XScale='log';
% han.sfig(kr,kc).YScale='log';
han.sfig(kr,kc).Title.String=labels4all('measurement',1,lan);
% han.sfig(kr,kc).XColor='r';
% han.sfig(kr,kc).YColor='k';
