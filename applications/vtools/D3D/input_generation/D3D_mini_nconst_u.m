...
%$Author: chavarri $
%$Id: D3D_mini_nconst_u.m 16571 2020-09-08 12:39:17Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/input_generation/D3D_mini_nconst_u.m $
%
...
function D3D_mini_nconst_u(simdef)
%% RENAME
...
data{4  ,1}=        'SubVersionInfo  = $Id: D3D_mini_nconst_u.m 16571 2020-09-08 12:39:17Z chavarri $';
data{5  ,1}=        'FileCreationDate= today :D';

%active layer
data{6  ,1}=        '[Layer]';
data{7  ,1}=        'Type            = volume fraction';
data{8  ,1}=sprintf('Thick           = %s_%02d.xyz','lyr',1);
for kf=1:nef+1
    data{8+kf,1}=sprintf('Fraction%d       = %s_%02d_%s_%02d.xyz',kf,'lyr',1,'frc',kf);
end
...
for ku=1:MxNULyr
    ...
    data{stu+(ku-1)*nlb+ku+2,1}=sprintf('Thick           = %s_%02d.xyz','lyr',ku+1);
    for kf=1:nef+1
        data{stu+(ku-1)*nlb+ku+2+kf,1}=sprintf('Fraction%d       = %s_%02d_%s_%02d.xyz',kf,'lyr',ku+1,'frc',kf);
    end
end
...
data{stf+3,1}=sprintf('Thick           = %s_%02d.xyz','lyr',MxNULyr+2);
for kf=1:nef+1
    data{stf+3+kf,1}=sprintf('Fraction%d       = %s_%02d_%s_%02d.xyz',kf,'lyr',MxNULyr+2,'frc',kf);
end
