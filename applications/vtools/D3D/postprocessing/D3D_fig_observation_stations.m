...
%$Author: chavarri $
%$Id: D3D_fig_observation_stations.m 17855 2022-03-25 06:31:33Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/postprocessing/D3D_fig_observation_stations.m $
%
%MATLAB BUGS:
...
function D3D_fig_observation_stations(in_p)
...
xlabels{kr,kc}=labels4all('x',1,'en');
ylabels{kr,kc}=labels4all('y',1,'en');
...
OPT.xlim=min(obs_sta.x);
OPT.ylim=max(obs_sta.y);
...
kr=1; kc=1;    
scatter(obs_sta.x,obs_sta.y,'parent',han.sfig(kr,kc));
axis equal
ns=numel(obs_sta.x);
for ks=1:ns
text(obs_sta.x(ks),obs_sta.y(ks),strrep(obs_sta.name{ks},'_','\_'));
end
