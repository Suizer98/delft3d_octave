...
%$Author: chavarri $
%$Id: D3D_figure_2DV_u.m 16571 2020-09-08 12:39:17Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/postprocessing/D3D_figure_2DV_u.m $
%
%plot of 1D volume fraction 2

function D3D_figure_2DV_u(simdef,in)
...
%figure input
prnt.filename=sprintf('v_%010.0f',time_r./flg.plot_unitt); %time is already in the plotting unitsm here we recover [s]
...
for kl=1:10
    for kp=1:np
        uc_loc=uc_norm(kl,kp);
        c_frac=(uc_loc-(uc_lim(1)-1e-4))/((uc_lim(2)+1e-4)-uc_lim(1));
%         c_frac=1.00001-c_frac; %comment off if velocities are positive and viceversa
        c_idx=ceil((c_frac)*(ncmap));

        quiver3(x_face_s(kp),y_face_s(kp),z_face_s(kl,kp),ucx_s(kl,kp),ucy_s(kl,kp),ucz_s(kl,kp),'color',cmap(c_idx,:))
    end
end
