...
%$Author: chavarri $
%$Id: D3D_fig_cmp_hisc.m 17473 2021-09-01 14:26:00Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/postprocessing/D3D_fig_cmp_hisc.m $
%
%MATLAB BUGS:
...
function D3D_fig_cmp_hisc(in_p)
% 
v2struct(in_p)
...
% prop.color = linspecer(length(in_p.sim_names),'sequential');
% prop.color=[... %<  matlab 2014b default
...
lims.c(kr,kc,1:2)=[min(time_r),max(time_r)];
xlabels{kr,kc}=val_label;
ylabels{kr,kc}='number of simulations [-]';

% lims_d.x(kr,kc,1:2)=seconds([3*3600+20*60,6*3600+40*60]); %duration
% lims_d.x(kr,kc,1:2)=[datenum(1998,1,1),datenum(2000,01,01)]; %time

% brewermap('demo')
cmap=brewermap(nt,'spectral');
...
for kt=1:nt-1
cen=cor2cen(edg(:,kt));
plot(cen,cou(:,kt),'parent',han.sfig(kr,kc),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1,'color',cmap(kt,:));
end
kt=nt;
cen=cor2cen(edg(:,kt));
plot(cen,cou(:,kt),'parent',han.sfig(kr,kc),'linewidth',prop.lw1*2,'linestyle',prop.ls1,'marker',prop.m1,'color',cmap(kt,:));

% han.p(kr,kc,:)=plot(datetime(tim_v,'convertFrom','datenum')-datetime(tim_v(1),'convertFrom','datenum'),norm_sim,'parent',han.sfig(kr,kc),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
% han.p(kr,kc,:)=plot(seconds(tim_v),norm_sim,'parent',han.sfig(kr,kc),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
...
%duration ticks
% kr=1; kc=1;
% xtickformat(han.sfig(kr,kc),'dd:hh:mm:ss')
...
% if size(norm_sim,1)>1
% han.leg(kr,kc)=legend(han.sfig(kr,kc),reshape(han.p(kr,kc,:),1,numel(han.p(kr,kc,:))),sim_names,'location','eastoutside');
% end
