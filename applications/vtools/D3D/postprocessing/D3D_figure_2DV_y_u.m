...
%$Author: chavarri $
%$Id: D3D_figure_2DV_y_u.m 16571 2020-09-08 12:39:17Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/postprocessing/D3D_figure_2DV_y_u.m $
%
%plot of 1D volume fraction 2

function D3D_figure_2DV_y_u(simdef,in)
...
%figure input
prnt.filename=sprintf('vy_%010.0f',time_r./flg.plot_unitt); %time is already in the plotting unitsm here we recover [s]
...
cbar.label='crosswise velocity [m/s]';
...
for kl=1:nl
    for kp=1:np
        switch flg.which_s 
            case 3
                scatter3(x_face_s(kp),y_face_s(kp),z_face_s(kl,kp),20,ucy_s(kl,kp),'filled')        
            case 5
                text(x_face_s(kp),y_face_s(kp),z_face_s(kl,kp),sprintf('%0.2e',ucy_s(kl,kp)))
        end        
    end
end
...
%         trisurf(tri,x_face_s(kp),y_face_s(kp),z_face_s(kp),ucz_s(kl,kp));
        ...
        surf(repmat(y_face_s,nl,1),z_face_s,ucy_s)
        ylabel('elevation [m]')
