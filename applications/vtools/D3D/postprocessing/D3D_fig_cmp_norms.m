...
%$Author: chavarri $
%$Id: D3D_fig_cmp_norms.m 17473 2021-09-01 14:26:00Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/postprocessing/D3D_fig_cmp_norms.m $
%
%MATLAB BUGS:
...
function D3D_fig_cmp_norms(in_p)
...
sim_v_c=cen2cor(1:length(sim_v));
[x_m,y_m,nodes]=meshgridNodes(sim_v_c,sim_v_c);
...
% kr=1; kc=1;
lims.y(kr,kc,1:2)=[min(sim_v_c),max(sim_v_c)];
lims.x(kr,kc,1:2)=[min(sim_v_c),max(sim_v_c)];
% lims.c(kr,kc,1:2)=clims;
xlabels{kr,kc}='simulation [-]';
ylabels{kr,kc}='simulation [-]';
...
kr=1; kc=1;    
% pcolor(B_m,'parent',han.sfig(kr,kc))
patch('faces',nodes','vertices',[x_m(:),y_m(:)],'FaceVertexCData',reshape(B_m,[],1),'FaceColor','flat','parent',han.sfig(kr,kc));
...
han.sfig(kr,kc).XTick=1:length(sim_v);  
han.sfig(kr,kc).YTick=1:length(sim_v);  
han.sfig(kr,kc).XTickLabel=sim_v;
han.sfig(kr,kc).YTickLabel=sim_v;
han.sfig(kr,kc).XTickLabelRotation = 90;
% han.sfig(kr,kc).XScale='log';
% han.sfig(kr,kc).YScale='log';
han.sfig(kr,kc).Title.String=tit;
