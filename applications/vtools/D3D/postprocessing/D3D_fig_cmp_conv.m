...
%$Author: chavarri $
%$Id: D3D_fig_cmp_conv.m 17473 2021-09-01 14:26:00Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/postprocessing/D3D_fig_cmp_conv.m $
%
%MATLAB BUGS:
...
function D3D_fig_cmp_conv(in_p)
...
kr=1; kc=1;    
for kp=1:np
han.p(kr,kc,1)=scatter(dx(kp),norms(kp),prop.ms1,marker_v{conv_categ(kp)},'filled','parent',han.sfig(kr,kc),'markerfacecolor',prop.mf1);
end
...
%for legend
ncat=numel(conv_categ_names);
for kcat=1:ncat
han.pl(kr,kc,kcat)=scatter(0,0,prop.ms1,marker_v{kcat},'filled','parent',han.sfig(kr,kc),'markerfacecolor',prop.mf1);
end
...
han.leg(kr,kc)=legend(han.sfig(kr,kc),reshape(han.pl(kr,kc,:),1,[]),conv_categ_names,'location','best');
