...
%$Author: chavarri $
%$Id: D3D_figure_modify_upstream_discharge.m 17536 2021-10-29 19:38:05Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/fcn/D3D_figure_modify_upstream_discharge.m $
%
%MATLAB BUGS:
...
function D3D_figure_modify_upstream_discharge(in_p)
...
kr=1; kc=1;    
han.p(kr,kc,1)=plot(s,Q,'parent',han.sfig(kr,kc),'color',cmap(1,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1{1});
han.p(kr,kc,2)=plot(s,Q_m,'parent',han.sfig(kr,kc),'color',cmap(2,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1{2});
...
han.leg(kr,kc)=legend(han.sfig(kr,kc),reshape(han.p(kr,kc,:),1,[]),{labels4all('original',1,lan),labels4all('modified',1,lan)},'location','best');
