...
%$Author: chavarri $
%$Id: D3D_io_xydata_adapted.m 16571 2020-09-08 12:39:17Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/fcn/D3D_io_xydata_adapted.m $
%
function varargout=D3D_io_xydata_adapted(cmd,varargin)
...
%           ncols = 2;
       ...
       elseif ~ischar(LINE(iline).DATA{1,1})
               % several columns 
               for irow = 1: size(LINE(iline).DATA,1)
                   fprintf(fid,strcat(repmat('%14.9e ',1, ncols),' \n'),LINE(iline).DATA{irow,:});
               end
       else
           if ischar(LINE(iline).DATA{1,1})
               % Date Time value
