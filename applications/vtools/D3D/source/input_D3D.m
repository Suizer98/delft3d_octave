...
%$Author: chavarri $
%$Id: input_D3D.m 17759 2022-02-14 09:50:54Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/D3D/source/input_D3D.m $
%
%NOTES:
%   -DIFFUSION
...
simdef.grd.K=               3; %number of vertical layers for flow [-] [double(1,1)] e.g. [2], 0=2D flow
...
simdef.grd.L=               10; %domain length [m] [10,100]
simdef.grd.B=               1.5; %width [m]; [0.01,0.05,0.1,0.2,1.0,10]
simdef.grd.dx=              0.25; %dx [m]; [0.01,0.05,0.1,0.2,1.0,10]
simdef.grd.dy=              0.20; %dy [m]; [0.01,0.05,0.1,0.2,1.0,10]
    %type 2
...
simdef.mdf.Tstop=           600; %simulation time [s] 
simdef.mdf.Dt=              30; %time step [s] [dx,dt]=[10,10]; [1,2.5]; [0.2,0.5]; [0.1,0.2]; [0.05,0.1]; [0.01,0.02];
simdef.mdf.FrictType=       10; %friction type [-]: 0=Chezy; 1=Manning; 2=White-Colebrook; 3=idem, WAQUA style; 10=smooth wall (no friction coefficient needed)
...
simdef.mdf.correct_C=       0; %correct friction for the z0r stuff bug: 0=NO; 1=YES [double(1,1)] e.g. [0]
...
simdef.mdf.Dicouv=          1.6907e-04; %horizontal eddy diffusivity [m^2/s] [double(1,1)] e.g. [5]
...
simdef.mor.CmpUpd=          1; %composition update [-] [double(1,1)]: 0=NO; 1=YES
...
%% initial conditions
    %etaw
simdef.ini.etaw_type=       2; %type of initial water level: 1=use flow depth; 2=read from file (only for FM);
...
simdef.mor.IBedCond=        3; %flag for upstream boundary bed boundary condition: 
                               %1=bed level fixed; 
...
simdef.tra.IFORM=           4; %sediment transport flag [-] [integer(1,1)] 2=MPM; 4=MPM-based; 14=AM
