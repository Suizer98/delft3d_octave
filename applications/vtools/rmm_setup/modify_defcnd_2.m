...
%$Author: chavarri $
%$Id: modify_defcnd_2.m 16571 2020-09-08 12:39:17Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/rmm_setup/modify_defcnd_2.m $
%

function modify_defcnd_2(path_file,defcnd_1,bcn,loclabels)

%% OPEN FILES

[folder_in,fname_in,ext_in]=fileparts(path_file);
path_aux=fullfile(folder_in,'DEFCND_aux.2');
...
while ~feof(fID_in)
    fline=fgets(fID_in);
    tok=regexp(fline,'FLBO id ''(\w+)''','tokens'); %search for FLBO id
    if isempty(tok) %if not found, write the line
       fprintf(fID_out,'%s',fline); 
    else %if dealing with a FLBO
        idx_defcnd=find(strcmp(tok{1,1}{1,1},{defcnd_1.FLBO.id}));
        idx_loc_aux=find(strcmp(defcnd_1.FLBO(idx_defcnd).ci,{loclabels.sre}));
        
        fprintf(fID_out,'%s',fline);
        if ~isempty(idx_loc_aux) %if dealing with controller we have to modify            
            ...
            %CHECK modify_defcnd_6. It would be better if the option '-'
            %above is not allowed and the user is forced to specify which
            %type of BC is he/she dealing with. Then, you pass below if
            ...
            for kidxbcn=1:nidxbcn
                ...
                if strcmp(paramcode,'WATHTE') || strcmp(paramcode,'Q')
