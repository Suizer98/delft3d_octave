...
%$Author: chavarri $
%$Id: modify_defcnd_6.m 16571 2020-09-08 12:39:17Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/rmm_setup/modify_defcnd_6.m $
%

function modify_defcnd_6(path_file,defcnd_1,bcn,loclabels)

%% OPEN FILES

[folder_in,fname_in,ext_in]=fileparts(path_file);
path_aux=fullfile(folder_in,'DEFCND_aux.6');
...
while ~feof(fID_in)
    fline=fgets(fID_in);
    tok=regexp(fline,'STBO id ''(\w+)''','tokens'); %search for FLBO id
    if isempty(tok) %if not found, write the line
       fprintf(fID_out,'%s',fline); 
    else %if dealing with a STBO
        idx_defcnd=find(strcmp(tok{1,1}{1,1},{defcnd_1.STBO.id}));
        idx_loc_aux=find(strcmp(defcnd_1.STBO(idx_defcnd).ci,{loclabels.sre})); 
        
        if isempty(idx_loc_aux) %if dealing with controller it is not in our list
            fprintf(fID_out,'%s',fline);
            ...
            if isnan(idx_loc) %if dealing with controller we do not have to modify             
                ...
                tokbc=regexp(fline,'co (\d)','tokens'); 
                fline=strrep(fline,sprintf('co %s',tokbc{1,1}{1,1}),'co 1');
