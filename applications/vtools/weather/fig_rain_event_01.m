...
%$Author: chavarri $
%$Id: fig_rain_event_01.m 18311 2022-08-19 04:18:42Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/weather/fig_rain_event_01.m $
%
%MATLAB BUGS:
...
function fig_rain_event_01(in_p)
...
if isfield(in_p,'fig_size')==0
    in_p.fig_size=[0,0,5,7]; %(1+sqrt(5)/2)
end
...
kr=1; kc=1;    
han.p1(kr,kc,1)=plot(mod_tim,mod_val,'.-b');
han.p1(kr,kc,2)=plot(mod_tim,mea_val.*ones(size(mod_tim)),'--r');
% scatter(mea_tim,mea_val,10,'r','filled')
scatter(mod_tim(end),mea_val,10,'r','filled')
...
han.leg(kr,kc)=legend(han.sfig(kr,kc),reshape(han.p1(kr,kc,:),1,[]),{'predicted','measured'},'location','best');
