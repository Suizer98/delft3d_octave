...
%$Author: chavarri $
%$Id: fig_qt.m 18311 2022-08-19 04:18:42Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/vtools/data_stations/fig_qt.m $
%
%MATLAB BUGS:
...
%figure input
prnt.filename=sprintf('qt_%s_%s_%d',station,station_q,numb);
...
ylabels{kr,kc}=sprintf('water elevation at %s [m]',station);
lims_d.x(kr,kc,1:2)=lim_x_qt; %time

kr=1; kc=2;
% lims.y(kr,kc,1:2)=[-2e-3,2e-3];
% lims.x(kr,kc,1:2)=lim_x;
% lims.c(kr,kc,1:2)=clims;
ylabels{kr,kc}=sprintf('water discharge at %s [m^3/s]',station_q);
% xlabels{kr,kc}=sprintf('water elevation at %s [m]',station);
lims_d.x(kr,kc,1:2)=lim_x_qt; %time
...
han.sfig(kr,kc).YColor=prop.color(1,:);

%plots
kr=1; kc=1;    
han.p(kr,kc,1)=plot(time_d,qh(:,1)./100,'parent',han.sfig(kr,kc),'color',prop.color(1,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);

kr=1; kc=2;    
han.p(kr,kc,1)=plot(time_d,qh(:,2),'parent',han.sfig(kr,kc),'color',prop.color(2,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
...
% han.p2(kr,kc,kd)=plot(q_u_mean{1,kd}(:,1),q_u_mean{1,kd}(:,2)./100,'parent',han.sfig(kr,kc),'color',prop.color2(kd,:),'linewidth',prop.lw1,'linestyle',prop.ls1{kd},'marker',prop.m1);
% % pause
% end
%duration ticks
...
% han.leg(kr,kc)=legend(han.sfig(kr,kc),[reshape(han.p3(kr,kc,:),1,numel(han.p3(kr,kc,:))),reshape(han.p2(kr,kc,:),1,numel(han.p2(kr,kc,:)))],{'before','after','mean before','mean after'},'location','southeast');
