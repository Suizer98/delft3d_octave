...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%
function [forLegend, boxEdge, wisEdge] = bplot_V(x,varargin)
...
if size(x,1)>1 && size(x,2)>1 % great, you want to plot a bunch.
    if isempty(varargin)
        forLegend = bplot_V(x(:,1),1);
        for ii=2:size(x,2)
            hold on;
            bplot_V(x(:,ii),ii,'nolegend');
        end
    else
        if ~ischar(varargin{1})
            warning('You can''t specify a location for multiple guys, this will probably crash');
        end
        forLegend = bplot_V(x(:,1),1,varargin{:});
        for ii=2:size(x,2)
            hold on;
            bplot_V(x(:,ii),ii,'nolegend',varargin{:});
