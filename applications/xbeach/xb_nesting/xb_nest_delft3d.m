function info = xb_nest_delft3d(varargin)
...
%
%   See also xb_nest_xbeach, xb_delft3d_flow, xb_delft3d_wave, xb_delft3d_bathy
...
% $Id: xb_nest_delft3d.m 4036 2011-02-15 15:32:35Z hoonhout $
% $Date: 2011-02-15 23:32:35 +0800 (Tue, 15 Feb 2011) $
% $Author: hoonhout $
% $Revision: 4036 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/xbeach/xb_nesting/xb_nest_delft3d.m $
% $Keywords: $
...
switch OPT.type
    ...
    case 'delft3d'
        ...
        info = struct('bctfile','','sp2file','','depfile','');
        
        % nest flow
        if ~isempty(OPT.flow)
            info.bctfile = xb_delft3d_flow(OPT.flow{:}, 'type', 'delft3d');
        end

        % nest wave
        if ~isempty(OPT.wave)
            info.sp2file = xb_delft3d_wave(OPT.wave{:}, 'type', 'delft3d');
        end

        % nest bathy
        if ~isempty(OPT.bathy)
            info.depfile = xb_delft3d_bathy(OPT.bathy{:}, 'type', 'delft3d');
        end
