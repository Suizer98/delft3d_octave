function output = xb_output_read(varargin)
%XB_OUTPUT_READ  Simple routine for first basic inspection of XBeach results
%
%   Simple routine for first basic inspection of XBeach results.
%
%   Syntax:
%   output = xb_output_read(varargin)
%
%   Input:
%   varargin  =
%
...
% $Id: xb_output_read.m 3489 2010-12-02 13:36:58Z heijer $
% $Date: 2010-12-02 21:36:58 +0800 (Thu, 02 Dec 2010) $
% $Author: heijer $
% $Revision: 3489 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/xbeach/.old/xb_output_read.m $
% $Keywords: $
clc
%% settings
% defaults
...
for i   = 1:length(OPT.var)
    ...
    for j = 1 : nt;
        if ismember(j,tdim)
            cntr = cntr + 1;
            output.(OPT.var{i})(:,:,cntr) = fread(fid,[nx+1,ny+1],'double');
        else
            fread(fid,[nx+1,ny+1],'double');
        end
        if j==1 && strcmpi(OPT.var{i}, 'zb')
            output.z0 = output.(OPT.var{i})(:,:,cntr);
        end
    end
end
