function []=UBtransportbarplot(PRNfile,varargin)
%UBtransportbarplot : Plots a barplot of the UNIBEST-CL results for alongshore sediment transport 
%   
%   Syntax:
%     function []=UBtransportbarplot(PRNfile,PRNreffile,settings)
%   
%   Input:
...
%   
%   Example:
%     []=UBtransportbarplot(PRNfile,PRNreffile,settings)
%   
%   See also UBtransportlineplot UBbarplot UBlineplot UBcoastlineplot UBcoastlinezoom
...
% $Id: UBtransportbarplot.m 3496 2013-02-06 15:24:35Z huism_b $
% $Date: 2013-02-06 16:24:35 +0100 (Wed, 06 Feb 2013) $
% $Author: huism_b $
% $Revision: 3496 $
% $HeadURL: https://repos.deltares.nl/repos/mctools/trunk/matlab/applications/UNIBEST_CL/postprocess/UBtransportbarplot.m $
% $Keywords: $
...
while NiOKAY==0
    aaa=[];
    data = struct;
    xi = xdist(1):(xdist(end)-xdist(1))/settings.Ni:xdist(end);
    transporti = interp1(xdist,transport,xi);
    for m = 1:length(xi)-1
        data(m).id = find(xdist >= xi(m) & xdist <= xi(m+1));            %find data points in section
...
for m = 1:length(xi)-1 
    ...
    data(m).xdiff = diff(xdist(data(m).id));                       %determine intervals
    ...
    data(m).transportii = interp1(xdist,transport,data(m).xii);         %perform interp
    data(m).xmean = mean(data(m).xii);                              %mean x of each sect
    data(m).transportmean = mean(data(m).transportii);                              %mean dz of each sect
    data(m).transportstd = std(data(m).transportii);                                %std dz of each sect
    xpl(m) = data(m).xmean;
    transportpl(m) = data(m).transportmean;
end
pos = find(transportpl>=0);
neg = find(transportpl<=0);
...
if strfind(lower(settings.orientation),'hor')
    if ~isempty(pos)
        bar(xpl(pos),transportpl(pos),settings.poscolor);hold on
    end
    if ~isempty(neg)
        bar(xpl(neg),transportpl(neg),settings.negcolor);hold on
    end
    plot(xpl,nul,'k')
    if      isfield(settings,'ylim') & ~isempty(settings.ylim)
            ylim(settings.ylim);
    else    ylim([min(transportpl)-settings.offset max(transportpl)+settings.offset]);
    end
    ...
    if ~isempty(pos)
        barh(xpl(pos),transportpl(pos),settings.poscolor);hold on
    end
    if ~isempty(neg)
        barh(xpl(neg),transportpl(neg),settings.negcolor);hold on
    end
    plot(nul,xpl,'k')
    set(gca,'Yticklabel',[]);
    set(gca,'YTick',[]);
    ...
    if      isfield(settings,'xlim') & ~isempty(settings.xlim)
            xlim(settings.xlim);
    else    xlim([min(transportpl)-settings.offset max(transportpl)+settings.offset]);
    end
