function [hline,zminz0] =UBlineplot(PRNfile,varargin)
%UBlineplot : Plots a line of the UNIBEST-CL results for changes in coastline position 
%   
%   Syntax:
%     function []=UBlineplot(PRNfile,PRNreffile,settings)
%   
%   Input:
...
%   
%   Example:
%     []=UBlineplot(PRNfile,PRNreffile,settings)
% 
%   Hint:
%     For plotting multiple lines in one plot, use 'hold on' and reuse function (if desired with different settings)
%   
%   See also UBbarplot UBtransportlineplot UBtransportbarplot UBcoastlineplot UBcoastlinezoom
...
% $Id: UBlineplot.m 3496 2013-02-06 15:24:35Z huism_b $
% $Date: 2013-02-06 16:24:35 +0100 (Wed, 06 Feb 2013) $
% $Author: huism_b $
% $Revision: 3496 $
% $HeadURL: https://repos.deltares.nl/repos/mctools/trunk/matlab/applications/UNIBEST_CL/postprocess/UBlineplot.m $
% $Keywords: $
...
% Find zminz0 for year to plot and if zoombox, adjust x- and transport-values accordingly
if      isfield(settings, 'xbox') & isfield(settings, 'ybox') & length(settings.xbox) == 2 & length(settings.ybox) == 2
...
nul = zeros(length(xdist2),1);
%xdist2 = maxX-xdist2;

% Plot-settings for different orientations
...
if      isfield(settings,'ylim') & ~isempty(settings.ylim)
        ylim(settings.ylim);
else    ylim([min(yplot)-settings.offsety max(max(yplot),1)+settings.offsety]);
end
if      isfield(settings,'xlim') & ~isempty(settings.xlim)
        xlim(settings.xlim);
else    xlim([min(xplot)-settings.offsetx max(max(xplot),1)+settings.offsetx]);
end
xl = xlim; yl = ylim;

%Check whether highlighted areas should be plotted
if  isfield(settings,'xid_hlight')
    xid_hlight = settings.xid_hlight;
    %If zoombox, adjust xid_hlight
    if strcmp(ZOOM,'ON')==1
        for mm = 1:length(xid_hlight)
            for jj=1:2
                kk = find(polid2 == xid_hlight{mm}{jj});
                if ~isempty(kk)
                    xid_hlight{mm}{jj} = kk;
                else
                    xid_hlight{mm}{jj} = -9999;
                end
            end
        end
    end
    ...
    if  strfind(lower(settings.orientation),'hor')
        ...
        for mm = 1:length(xid_hlight)
            for jj=1:2
                if xid_hlight{mm}{jj} ~= -9999
                    plotX{mm,jj} = [xdist2(xid_hlight{mm}{jj}),xdist2(xid_hlight{mm}{jj})];
    ...
    %For vertical orienation    
        reflimits = [yl(1),yl(2)];
        for mm = 1:length(xid_hlight)
            for jj=1:2
                if xid_hlight{mm}{jj} ~= -9999
                    plotY{mm,jj} = [xdist2(xid_hlight{mm}{jj}),xdist2(xid_hlight{mm}{jj})];
    ...
    for mm = 1:length(xid_hlight)
        for jj=1:2
            %Plot hlight boundaries
            if  xid_hlight{mm}{jj} ~= -9999 && xdist2(xid_hlight{mm}{jj})>reflimits(1) && xdist2(xid_hlight{mm}{jj})<reflimits(2)
                plot([plotX{mm,jj},plotX{mm,jj}],[plotY{mm,jj},plotY{mm,jj}],'Color',[0.7 0.7 0.7],'LineWidth',min(max(settings.linewidth-1,0.5),1))
            end
        end
        %Plot hlight fill
        if  strcmp(ZOOM,'ON')==0
            %fill([plotfill{mm}(1),plotfill{mm}(2),plotfill{mm}(2),plotfill{mm}(1),plotfill{mm}(1)],[plotfill{mm}(3),plotfill{mm}(3),plotfill{mm}(4),plotfill{mm}(4),plotfill{mm}(3)],[0.7 0.7 0.7]);
        end         
    end
end
