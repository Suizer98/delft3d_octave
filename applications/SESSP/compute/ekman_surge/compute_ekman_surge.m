function surge=compute_ekman_surge(x,t,vmax,rmax,r35,foreward_speed,phi,phi_spiral,shelf_width,a,manning,latitude,varargin)
...
%    cpx=compute_ekman_surge_cpx(a,vmax,shelf_width,foreward_speed,phi,r35);
...
%    a1n=compute_ekman_surge_a1n(latitude,r35,foreward_speed,phi,a);
...
% semaxt=semax*e*(-(t-t0)/cpt).*exp(-(-(t-t0)/cpt).^1);
% semaxt(t>t0)=0;
% 
% cnt=t0n-t0;
% semint=semin*e*((t-t0)/cnt).*exp(-((t-t0)/cnt).^1);
% semint(t<t0)=0;
