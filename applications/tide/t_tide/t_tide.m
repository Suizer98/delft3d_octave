...
%              29/09/14 - numerical typo in MAD to STD conversion corrected!
...
%                Oct/22 - backported Rich's v1.4 (2019) into the OET version

% Version 1.4
% $Id: t_tide.m 18453 2022-10-14 12:42:33Z l.w.m.roest.x $
        ...
        switch lower(varargin{1}(1:3))
            ...
            case 'ray'
                if isnumeric(varargin{2})
                    ...
                    if iscellstr(constitnames) %#ok<ISCLSTR>
...
nreal=1;
if any(strcmp(errcalc(2:end),'boot')) && exist('dt0','var')
    ...
    epsp = zeros(1,nreal);
    epsm = zeros(1,nreal);
elseif any(strcmp(errcalc(2:end),'boot')) && ~exist('dt0','var')
    if ~diar==0
    ...
    ap=abs(AP);
    am=abs(AM);
elseif any(strncmpi(errcalc,'linear',3)) || exist('dt0','var') % non-equidistant time series do not allow for fft
    if ~diar==0
...
if fid>0
    fprintf(fid,'t_tide version: $Id: t_tide.m 18453 2022-10-14 12:42:33Z l.w.m.roest.x $ \n');
    ...
    if isreal(xin)
        ...
        for k=1:length(fu);
            ...
            fprintf(fid,'%s %9.7f %7.3f %7.3f %8.3f %7.3f %7.2f %7.2f %8.2f %8.2f %8.2g\n',...
                nameu(k,:),fu(k),tidecon(k,:),snr(k));
        end
...
if minres>1/(18.6*365.25*24)                       % Choose only resolveable pairs for short
    [const,~,~]=t_getconsts(centraltime); % Time series
...
%    [const,sat,cshallow]=t_get18consts(centraltime); % Function does not exist, load matfile instead.
...
if ~isempty(constit)     % Selected if constituents are specified in input.
    ...
    for k=1:size(constit,1)
        j1=find(strcmp(constit(k,:),cellstr(const.name)));
        if isempty(j1)
...
if ~isempty(shallow)          % Add explictly selected shallow water constituents.
    for k=1:size(shallow,1)
        j1=find(strcmp(shallow(k,:),cellstr(const.name)));
        if isempty(j1)
...
if ~isempty(infname)
    ...
    for k=1:size(infname,1)
        ...
        if isempty(j1)
            ...
            j1=find(strcmp(infref(k,:),cellstr(nameu)));
            if isempty(j1)
                disp(['Can''t recognize name ' infref(k,:) ' for as a reference for inference']);
            else
                jref(k)=j1;
                fprintf('   Inference of %s using  %s\n',namei(k,:),nameu(j1,:))
                %       if ~diar==0
...
NP=NM;
for k=1:length(fu)
    l=fu(k)>fband(:,1) & fu(k)<fband(:,2); %find(fu(k)>fband(:,1) & fu(k)<fband(:,2));
