% This scripts prepares the measured data (netcdf files with data from
% waterbase) for GoFStats_v3.m: one column with daynumber and one column
% with measured data.
% data: 31/07/2012
% author: Valesca Harezlak (valesca.harezlak@deltares.nl)
% TODO: filter on empty text files....now have to delete them by hand
...
loaparent= 'd:\GEM-Egmond\meetdata\Gof_Files\meetdata\';
loadir='Gof_Files';
...
for l=1:length(req_sub) % substance selection
    a=l+2;
    ...
    for i=1:length(river)
        filename = [char(req_sub(l)),'_',char(river{i}),'_','monitor','.txt'];
        ...
        for z=year_start:year_end
            ...
            for j=1:length(new_datestr)
                fprintf(fid,'%d\t%d \n',Data.(river{i}).daynumber(j,:),...
                    Data.(river{i}).stof_new(j));
            end
