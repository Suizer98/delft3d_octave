...
% h_u=0.329168606246920*ones(1,nx)'; %0.20
h_u=[linspace(0.33,0.5,nx/2),linspace(0.5,0.33,nx/2+1)]'; %0.20
u=0.607591356540161*ones(1,nx)'; %0.70
cf_b=0.01*ones(1,nx)'; %bed
...
Fak=(1-0.14)*ones(1,nx)'; %[0.14,1-0.14]
Mak=Fak.*La;
Fr=u./sqrt(cnt.g.*h_u);
Sf=cf_f.*Fr.^2;
...
dh=1e-6;
[~,~,~,~,~,~,~,~,~,~,~,~,~,~,vpk_hdh,~,~]=sediment_transport(flg,cnt,h_u+dh,u.*h_u,cf_b,La,Mak,dk,sed_trans_param,hiding_param,mor_fac,E_param,vp_param,Gammak);
...
%initial condition
% C1=Gammak_eq(1)*1.0; %to compare to analytical
C1=1.963400567023249e-05; %[1.299628674097695e-05;1.963400567023249e-05]
...
switch input.bcm.pa_u
    case 1
        ...
        h_u=NaN(1,2);
    case 3
%         g_u=[-vpk(1)*C0,-vpk(1)*C0]; for comparison with analytical
        g_u=-[0.25,0.25]*1e-5;
        h_u=[vpk(1),vpk(1)];
    otherwise
        error('shit!')        
