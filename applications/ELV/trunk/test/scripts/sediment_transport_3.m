...
%$Author: chavarri $
%$Id: sediment_transport_3.m 16592 2020-09-16 17:32:43Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ELV/trunk/test/scripts/sediment_transport_3.m $
%
...
function [qbk,Qbk,thetak,qbk_st,Wk_st,u_st,xik,Qbk_st,Ek,Ek_st,Ek_g,Dk,Dk_st,Dk_g,vpk,vpk_st,Gammak_eq,Dm]=sediment_transport_3(flg,cnt,h,q,cf,La,Mak,dk,sed_trans_param,hiding_param,mor_fac,E_param,vp_param,Gammak,fid_log,kt)
...
switch flg.sed_trans
    case 1 %MPM48
        ...
        Qbk=a_mpm.*sqrt(cnt.g*cnt.R*dk.^3)./(1-cnt.p).*(mu.*thetak-xik.*theta_c).^b_mpm; %MPM relation
    ...
    case 6 %Parker
        ...
        Qbk=a_park.*sqrt(cnt.g*cnt.R*dk.^3)./(1-cnt.p).*thetak.^(3/2).*G;
        no_trans_idx=false(nx,nf);
    case 7 %Ribberink      
        ...
        Qbk=(1-0.40)/(1-cnt.p)*m_r.*(u.^n_r)./(Dm.^l_r); %Ribberink
        no_trans_idx=false(nx,nf);
    otherwise 
