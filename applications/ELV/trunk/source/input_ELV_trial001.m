...
input.mdv.flowtype=3; %flow assumption: 0=NO update 1=steady; 2=quasi-steady; 3=unsteady (explicit); 4=unsteady (implicit); [-]; [1x1 double]; e.g. [1]
...
input.mdv.Tstop=40*86400; %simulation time [s]; [1x1 double]; e.g. [3600]
input.mdv.dt_type=1; %type of time step method [-]: 1=fixed time step; 2=fixed CFL number; [1x1 double]; e.g. [1]  
input.mdv.dt=10; %time step [s]; [1x1 double]; e.g. [2] When using input.mdv.dt_type=2 this is the first time step.
...
input.mdv.Flmap_dt=864000; %printing map-file interval time [s]; [1x1 double]; e.g. [60]  
...
input.frc.Cf=0.00613; %friction coefficient g/C^2 [-]; [1x1 double]; e.g. [0.008]
...
input.grd.L=100000; %domain length [m]; [1x1 double]; e.g. [100]
input.grd.dx=250; %streamwise discretizations [m]; [1x1 double]; e.g. [0.1]
input.grd.B=100; %width [m]; [1x1 double] | [1xnx double]; e.g. [10]
...
input.mor.gsdupdate=0; %update grain size distribution 0=NO; 1=Hirano ;2=eli 1 (max. La); 3=eli 1 (min. La); 4=pmm (a<1,b~=1); 5=pmm (a>1,b~=1); 6=(a<1,b=1); 7=(a>1,b=1); 8=impose a (by means of pmm_alpha_eps); 9=a_m (M_eta=1/b, M_Fak=1/(a*b)); [-]; e.g. 1 
...
input.mor.La=1; %active layer thickness [m]; [1x1 double] | [nix1 double]; e.g. [0.1];
input.mor.ThUnLyr=1; %thickness of each underlayer [m]; [1x1 double]; e.g. [0.15]
...
input.tra.cr=2; %sediment transport closure relation 1=Meyer-Peter-Muller; 2=Engelund-Hansen; 3=Ashida-Michiue; 4=Wilcock-Crowe; 5=general power law; 6=Parker; 7=Ribberink [1x1 double]; e.g. [1]
...
input.tra.param=[17,0.05]; %sediment transport parameters (depending on the closure relation) 1=[8,1.5,0.047] (MPM) [5.7,1.5,0.047] (FLvB); 2=[0.05,5] (EH); 3=[17,0.05] (AM)
...
input.ini.initype=4; %kind of initial condition= %1=normal flow (for a given qbk0); 2=free; 3=from file; 4=normal flow (for a given initial condition) [-] [1x1 double]; e.g. [1]
...
input.ini.u=0.92831777; %flow velocity [m/s]; [1x1 double] | [1xnx double]; e.g. [1]
input.ini.h=5.3860867; %flow depth [m]; [1x1 double] | [1xnx double]; e.g. [1.5]
input.ini.slopeb=0.0001; %bed slope [-]; [1x1 double] | [1xnx double]; e.g. [1e-3]
...
input.ini.trench.x=[2000,2500]; %x coordinates of the trench (beginning and end) [m]; [1x2 double]; e.g. [5,7.5]
input.ini.trench.dz=+0.5; %trench depth (negative is dredge, positive is dump) [m]; [1x1 double]; e.g. [-0.04]
...
input.bch.Q0=[500;500]; %water discharge at the specified times [m^3/s]; [ntx1 double]; e.g. [1;2]
...
% input.bcm.NFLtype=1; %only if input.bcm.type=13 : 1=specify slope and GSD of the bed; 2=specify the total sediment load and the GSD of the bed; 3=specify sediment load
% input.bcm.NFLparam=[1e-4;1.0]; %if input.bcm.NFLtype=1: [slope,Fak (for all k)] e.g. [1e-4;0.1;0.9]; if input.bcm.NFLtype=2: [Qb,Fak (for all k)] e.g. [1e-5;0.1;0.9]; if input.bcm.NFLtype=3: [Qbk] e.g. [0.56;0.11]*1e9/2650/31536000
