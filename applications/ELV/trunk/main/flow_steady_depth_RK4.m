...
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ELV/trunk/main/flow_steady_depth_RK4.m $
%
...
R2=dx/2*dH_dx(H_ce+R1/2,qw(kx),Cf(kx),slopeb(kx),g);
R3=dx/2*dH_dx(H_ce+R2/2,qw(kx),Cf(kx),slopeb(kx),g);
R4=dx/2*dH_dx(H_ce+R3  ,qw(kx),Cf(kx),slopeb(kx),g);

H(1,kx)=H_ce-1/6*(R1+2*R2+2*R3+R4);
