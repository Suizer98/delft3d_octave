...
%$Author: chavarri $
%$Id: substrate_update.m 16592 2020-09-16 17:32:43Z chavarri $
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ELV/trunk/main/substrate_update.m $
%
...
%                 if nif==0 || kt<10 %there are no immobile fractions %200708 Why did I put 10 time steps?
                if nif==0 || kt<Tstart %there are no immobile fractions
                    ...
                    fIk_full(imm_idx)=1/nif; %immobile fractions are equally deposited
                    fIk(:,kx)=fIk_full(1:nef);
%                     % DEBUG
% fprintf('%10.9e \n',Fak_full)
% plot(Fak)
%                     fIk_imm=sum(fIk(imm_idx,kx)); %total volume fraction of immobile sediment
%                     if fIk(:,kx)>1+1e-7 || fIk(:,kx)<1-1e-7 
%                         error('mass issue')
%                     end
%                     % END DEBUG
                end
        ...
        if ec(kx)~=1 %more than 1 substrate layer
            ...
            %all the other cells
            if ec(kx)+cj(kx)<=nsl %the end cell has thickness 0; add one layer
                if cj(kx)<2 %when nf>2, the trick of using the empty array does not work. E.g. a(1,1,2:1)=5 -> ok;  a(1,1,2:1)=[5,5] -> error
