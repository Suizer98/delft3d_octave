...
%
%   Format Paramter:
%     'Format'  one of the strings 'eps','eps2','jpeg','png','tiff'
...
%     'Previewtiff'  'yes' or 'no' (default 'no')
%         switch to include previewtiff in EPS file.
%
...
for k = 1:2:length(paramPairs)
  ...
  switch (param)
   case 'format'
    format = value;
    if (~strcmp(format,{'eps','eps2','jpeg','png','tiff'}))
      error('Format must be ''eps'', ''eps2'', ''tiff'', ''jpeg'' or ''png''.');
    end
   case 'width'
	width = value;
   ...
   case 'previewtiff'
	previewtiff = lower(value);
	if (~strcmp(previewtiff,{'yes','no'}))
	  error('Previewtiff must be ''yes'' or ''no''.');
    end
    previewtiff = strcmp(previewtiff,'yes');
   case 'resolution'
	resolution = value;
...
% Add preview tiff when requested
if (previewtiff)
   args{end+1}='-tiff';
end
