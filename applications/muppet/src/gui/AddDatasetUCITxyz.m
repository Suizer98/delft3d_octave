function varargout = AddDatasetUCITxyz(varargin)
% ADDDATASETUCITXYZ M-file for AddDatasetUCITxyz.fig
%      ADDDATASETUCITXYZ, by itself, creates a new ADDDATASETUCITXYZ or raises the existing
%      singleton*.
%
%      H = ADDDATASETUCITXYZ returns the handle to a new ADDDATASETUCITXYZ or the handle to
%      the existing singleton*.
%
%      ADDDATASETUCITXYZ('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDDATASETUCITXYZ.M with the given input arguments.
%
%      ADDDATASETUCITXYZ('Property','Value',...) creates a new ADDDATASETUCITXYZ or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AddDatasetUCITxyz_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AddDatasetUCITxyz_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AddDatasetUCITxyz
 
% Last Modified by GUIDE v2.5 16-May-2007 13:28:19
 
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @AddDatasetUCITxyz_OpeningFcn, ...
                   'gui_OutputFcn',  @AddDatasetUCITxyz_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before AddDatasetUCITxyz is made visible.
function AddDatasetUCITxyz_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AddDatasetUCITxyz (see VARARGIN)
 
% Choose default command line output for AddDatasetUCITxyz
...
handles.FileInfo=load([handles.PathName handles.FileName]);

mmax=size(handles.FileInfo.d.X,1);
nmax=size(handles.FileInfo.d.X,2);
...
set(handles.SelectParameter,'Enable','off','BackGroundColor',[0.831 0.816 0.784]);
set(handles.TextParameter,'Enable','off');
set(handles.SelectBlock,'Enable','off','BackGroundColor',[0.831 0.816 0.784]);
set(handles.TextBlock,'Enable','off');
...
clear handles.FileInfo;

% Update handles structure
guidata(hObject, handles);
 
% UIWAIT makes AddDatasetUCITxyz wait for user response (see UIRESUME)
uiwait(handles.figure1);
 
 
% --- Outputs from this function are returned to the command line.
function varargout = AddDatasetUCITxyz_OutputFcn(hObject, eventdata, handles)
...
if NewDatasetName(handles.DataProperties,handles.NrAvailableDatasets,name)
    ...
    handles.DataProperties(nr).FileType='UCITxyz';
...
%     handles.DataProperties(nr).Column=get(handles.SelectParameter,'Value')+2;
    ...
    handles.DataProperties=ImportUCITxyz(handles.DataProperties,nr);
