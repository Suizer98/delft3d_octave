function varargout = AddDatasetSamples(varargin)
% ADDDATASETSAMPLES M-file for AddDatasetSamples.fig
%      ADDDATASETSAMPLES, by itself, creates a new ADDDATASETSAMPLES or raises the existing
%      singleton*.
%
%      H = ADDDATASETSAMPLES returns the handle to a new ADDDATASETSAMPLES or the handle to
%      the existing singleton*.
%
%      ADDDATASETSAMPLES('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDDATASETSAMPLES.M with the given input arguments.
%
%      ADDDATASETSAMPLES('Property','Value',...) creates a new ADDDATASETSAMPLES or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AddDatasetSamples_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AddDatasetSamples_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AddDatasetSamples
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @AddDatasetSamples_OpeningFcn, ...
                   'gui_OutputFcn',  @AddDatasetSamples_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before AddDatasetSamples is made visible.
function AddDatasetSamples_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AddDatasetSamples (see VARARGIN)
 
% Choose default command line output for AddDatasetSamples
...
handles.a=load([handles.PathName handles.FileName]);
...
%FileInfo=TEKAL('open',handles.fname);
...
% UIWAIT makes AddDatasetSamples wait for user response (see UIRESUME)
uiwait(handles.figure1);
 
 
% --- Outputs from this function are returned to the command line.
function varargout = AddDatasetSamples_OutputFcn(hObject, eventdata, handles)
...
if NewDatasetName(handles.DataProperties,handles.NrAvailableDatasets,name)
    ...
    handles.DataProperties(nr).FileType='Samples';
    ...
    handles.DataProperties(nr).Type='Samples';
 
    handles.DataProperties=ImportSamples(handles.DataProperties,nr);
...
set(handles.EditDatasetName,'String',str{i});
