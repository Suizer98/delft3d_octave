function varargout = AddDatasetSimonaSDS(varargin)
% ADDDATASETSIMONASDS M-file for AddDatasetSimonaSDS.fig
%      ADDDATASETSIMONASDS, by itself, creates a new ADDDATASETSIMONASDS or raises the existing
%      singleton*.
%
%      H = ADDDATASETSIMONASDS returns the handle to a new ADDDATASETSIMONASDS or the handle to
%      the existing singleton*.
%
%      ADDDATASETSIMONASDS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDDATASETSIMONASDS.M with the given input arguments.
%
%      ADDDATASETSIMONASDS('Property','Value',...) creates a new ADDDATASETSIMONASDS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AddDatasetSimonaSDS_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AddDatasetSimonaSDS_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AddDatasetSimonaSDS
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @AddDatasetSimonaSDS_OpeningFcn, ...
                   'gui_OutputFcn',  @AddDatasetSimonaSDS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before AddDatasetSimonaSDS is made visible.
function AddDatasetSimonaSDS_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AddDatasetSimonaSDS (see VARARGIN)

% Choose default command line output for AddDatasetSimonaSDS
...
%    case{'Delft3D-trim','Delft3D-com','Delft3D-hwgxy','Delft3D-waq-map'}
        ...
        handles.Stations=qpread(handles.FileInfo,'water level (station)','Stations');
...
% UIWAIT makes AddDatasetSimonaSDS wait for user response (see UIRESUME)
uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = AddDatasetSimonaSDS_OutputFcn(hObject, eventdata, handles) 
...
%     handles.SubField=handles.DataProps(i).SubFld;
    ...
    switch(lower(handles.Parameter))
    ...
    nt=length(handles.Times);
    if nt>0
        set(handles.TextTimes,'String',num2str(nt));
    else
        set(handles.TextTimes,'String','-');
    end
...
if NewDatasetName(handles.DataProperties,handles.NrAvailableDatasets,Name)
    ...
    %         otherwise
    handles.DataProperties(nr).FileType='SimonaSDS';
    %     end
    ...
    switch(lower(handles.Parameter))
        case{'stations','depth grid','bed level'}
            handles.DataProperties(nr).Station=[];
            handles.DataProperties(nr).DateTime=0;
            handles.DataProperties(nr).Block=0;
        otherwise
...
%     end
    ...
    wb = waitbox('Reading data file ...');pause(0.1);
    handles.DataProperties=ImportSimonaSDS(handles.DataProperties,nr);
    close(wb);
...
%     switch handles.FileInfo.SubType,
%         case{'Delft3D-trim'}
%             ii=strfind(handles.FileInfo.FileName,'trim');
%             RunidStr=[' - ' handles.FileInfo.FileName(ii+5:end)];
%         case{'Delft3D-trih'}
%             ii=strfind(handles.FileInfo.FileName,'trih');
%             RunidStr=[' - ' handles.FileInfo.FileName(ii+5:end)];
%     end
% else
    RunidStr='';
