function varargout = AddDatasetAnnotation(varargin)
% ADDDATASETANNOTATION M-file for AddDatasetAnnotation.fig
%      ADDDATASETANNOTATION, by itself, creates a new ADDDATASETANNOTATION or raises the existing
%      singleton*.
%
%      H = ADDDATASETANNOTATION returns the handle to a new ADDDATASETANNOTATION or the handle to
%      the existing singleton*.
%
%      ADDDATASETANNOTATION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDDATASETANNOTATION.M with the given input arguments.
%
%      ADDDATASETANNOTATION('Property','Value',...) creates a new ADDDATASETANNOTATION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AddDatasetAnnotation_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AddDatasetAnnotation_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AddDatasetAnnotation
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @AddDatasetAnnotation_OpeningFcn, ...
                   'gui_OutputFcn',  @AddDatasetAnnotation_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before AddDatasetAnnotation is made visible.
function AddDatasetAnnotation_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AddDatasetAnnotation (see VARARGIN)
 
% Choose default command line output for AddDatasetAnnotation
...
set(handles.SelectParameter,'String',handles.name);
...
% UIWAIT makes AddDatasetAnnotation wait for user response (see UIRESUME)
uiwait(handles.figure1);
 
 
% --- Outputs from this function are returned to the command line.
function varargout = AddDatasetAnnotation_OutputFcn(hObject, eventdata, handles)
...
if NewDatasetName(handles.DataProperties,handles.NrAvailableDatasets,name)
    ...
    handles.DataProperties(nr).FileType='Annotation';
    ...
    handles.DataProperties=ImportAnnotation(handles.DataProperties,nr);
