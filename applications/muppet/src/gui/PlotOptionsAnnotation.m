function varargout = PlotOptionsAnnotation(varargin)
% PLOTOPTIONSANNOTATION M-file for PlotOptionsAnnotation.fig
%      PLOTOPTIONSANNOTATION, by itself, creates a new PLOTOPTIONSANNOTATION or raises the existing
%      singleton*.
%
%      H = PLOTOPTIONSANNOTATION returns the handle to a new PLOTOPTIONSANNOTATION or the handle to
%      the existing singleton*.
%
%      PLOTOPTIONSANNOTATION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PLOTOPTIONSANNOTATION.M with the given input arguments.
%
%      PLOTOPTIONSANNOTATION('Property','Value',...) creates a new PLOTOPTIONSANNOTATION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PlotOptionsAnnotation_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PlotOptionsAnnotation_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PlotOptionsAnnotation
 
% Last Modified by GUIDE v2.5 30-May-2007 17:01:38
 
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PlotOptionsAnnotation_OpeningFcn, ...
                   'gui_OutputFcn',  @PlotOptionsAnnotation_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PlotOptionsAnnotation is made visible.
function PlotOptionsAnnotation_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PlotOptionsAnnotation (see VARARGIN)
 
% Choose default command line output for PlotOptionsAnnotation
...
set(handles.SelectMarkerEdgeColor,'String',handles.Colors);
i=strmatch(lower(handles.PlotOptions.MarkerEdgeColor),lower(handles.Colors),'exact');
set(handles.SelectMarkerEdgeColor,'Value',i);

set(handles.SelectMarkerFaceColor,'String',handles.Colors);
i=strmatch(lower(handles.PlotOptions.MarkerFaceColor),lower(handles.Colors),'exact');
...
% UIWAIT makes PlotOptionsAnnotation wait for user response (see UIRESUME)
uiwait(handles.figure1);
 
 
% --- Outputs from this function are returned to the command line.
function varargout = PlotOptionsAnnotation_OutputFcn(hObject, eventdata, handles)
...
h.Figure(i).Axis(j).Plot(k).MarkerEdgeColor=handles.Colors{get(handles.SelectMarkerEdgeColor,'Value')};
h.Figure(i).Axis(j).Plot(k).MarkerFaceColor=handles.Colors{get(handles.SelectMarkerFaceColor,'Value')};
