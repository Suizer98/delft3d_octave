function varargout = AddDatasetTekalXY(varargin)
% ADDDATASETTEKALXY M-file for AddDatasetTekalXY.fig
%      ADDDATASETTEKALXY, by itself, creates a new ADDDATASETTEKALXY or raises the existing
%      singleton*.
%
%      H = ADDDATASETTEKALXY returns the handle to a new ADDDATASETTEKALXY or the handle to
%      the existing singleton*.
%
%      ADDDATASETTEKALXY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDDATASETTEKALXY.M with the given input arguments.
%
%      ADDDATASETTEKALXY('Property','Value',...) creates a new ADDDATASETTEKALXY or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AddDatasetTekalXY_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AddDatasetTekalXY_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AddDatasetTekalXY
 
% Last Modified by GUIDE v2.5 11-Jul-2006 12:05:27
 
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @AddDatasetTekalXY_OpeningFcn, ...
                   'gui_OutputFcn',  @AddDatasetTekalXY_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before AddDatasetTekalXY is made visible.
function AddDatasetTekalXY_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AddDatasetTekalXY (see VARARGIN)
 
% Choose default command line output for AddDatasetTekalXY
...
for i=1:nrcolumns
    str{i}=FileInfo.Field(1).ColLabels{i};
    if size(str{i},2)==0
        str{i}=['Column ' num2str(i)];
    end
end
set(handles.SelectParameter,'String',str);
set(handles.SelectColumnX,'String',str);
...
set(handles.SelectColumnX,'Value',1);
set(handles.SelectParameter,'Value',2);
...
% UIWAIT makes AddDatasetTekalXY wait for user response (see UIRESUME)
uiwait(handles.figure1);
 
 
% --- Outputs from this function are returned to the command line.
function varargout = AddDatasetTekalXY_OutputFcn(hObject, eventdata, handles)
...
if NewDatasetName(handles.DataProperties,handles.NrAvailableDatasets,name)
    ...
    handles.DataProperties(nr).FileType='TekalXY';
    ...
    handles.DataProperties(nr).ColumnX=get(handles.SelectColumnX,'Value');
    handles.DataProperties(nr).Column=get(handles.SelectParameter,'Value');
    ...
    handles.DataProperties=ImportTekalXY(handles.DataProperties,nr);
...
% --- Executes on selection change in SelectColumnX.
function SelectColumnX_Callback(hObject, eventdata, handles)
% hObject    handle to SelectColumnX (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns SelectColumnX contents as cell array
%        contents{get(hObject,'Value')} returns selected item from SelectColumnX


% --- Executes during object creation, after setting all properties.
function SelectColumnX_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SelectColumnX (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
