function varargout = AddDatasetBar(varargin)
% ADDDATASETBAR M-file for AddDatasetBar.fig
%      ADDDATASETBAR, by itself, creates a new ADDDATASETBAR or raises the existing
%      singleton*.
%
%      H = ADDDATASETBAR returns the handle to a new ADDDATASETBAR or the handle to
%      the existing singleton*.
%
%      ADDDATASETBAR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDDATASETBAR.M with the given input arguments.
%
%      ADDDATASETBAR('Property','Value',...) creates a new ADDDATASETBAR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AddDatasetBar_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AddDatasetBar_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AddDatasetBar
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @AddDatasetBar_OpeningFcn, ...
                   'gui_OutputFcn',  @AddDatasetBar_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before AddDatasetBar is made visible.
function AddDatasetBar_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AddDatasetBar (see VARARGIN)
 
% Choose default command line output for AddDatasetBar
...
% UIWAIT makes AddDatasetBar wait for user response (see UIRESUME)
uiwait(handles.figure1);
 
 
% --- Outputs from this function are returned to the command line.
function varargout = AddDatasetBar_OutputFcn(hObject, eventdata, handles)
...
if NewDatasetName(handles.DataProperties,handles.NrAvailableDatasets,name)
    ...
    handles.DataProperties(nr).FileType='Bar';
    ...
    handles.DataProperties(nr).ColumnX=0;
    handles.DataProperties(nr).Column=get(handles.SelectParameter,'Value')+1;
    ...
    handles.DataProperties=ImportBar(handles.DataProperties,nr);
