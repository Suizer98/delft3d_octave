function msk = hurrywave_make_mask_advanced(x,y,z,varargin)
...
% boundary cells
zlev_polygon = 5; %max elevation to apply msk=2 to 'boundarypolygon' and minimum elevation to apply msk=3 for 'outflowboundarypolygon'
    ...
    if ischar(varargin{ii})
        switch lower(varargin{ii})
            ...
            case{'boundarypolygon'}
                xy_bnd = varargin{ii+1};
                count_boundarycells = count_boundarycells + 1;
                varargin_boundarycells(count_boundarycells).action = 'boundarypolygon';
...
%% 2. Determine msk=1/2/3 values for boundary cells
...
if count_boundarycells > 0
    ...
    for ii = 1:count_boundarycells %use order as defined in varargin to do this
        if strcmp(varargin_boundarycells(ii).action, 'boundarypolygon')
            
            if ~isempty(xy_bnd)
                msk = replace_boundary_cells(x,y,msk,msk_ids_edge,z,xy_bnd,zlev_polygon);
            else
                warning('boundarypolygon is empty')
            end
...
% Set outer edges of mask to 2 (boundary points)
msk_ids_edge(:,1)=1;
msk_ids_edge(:,end)=1;
msk_ids_edge(1,:)=1;
msk_ids_edge(end,:)=1;
...
%%
function msk = replace_boundary_cells(x,y,msk,msk_ids_edge,z,xy_poly,zlev_polygon)
disp('Debug - call replace_boundary_cells')

msk_ids = inpolygon_to_grid(x,y,xy_poly); %in & on are included by as standard

zlev_polygon_ids = z <= zlev_polygon; %only set to msk=2 if below/equal elevation of zlev_polygon (default is 5m)

msk_ids_bnd = msk_ids_edge == true & msk_ids == true & zlev_polygon_ids == true;

msk(msk_ids_bnd) = 2;

disp('Debug - finished replace_boundary_cells')
