...
%         t1=datenum(t1,'yyyymmdd');
        ...
        fname=[url '/' num2str(yr)];
        t1=nc_varget(fname,'time',0,1);
        t1=t1/24+datenum(2000,1,1);
...
%            data=nc_varget(fname,'temperature',[ndays 0 imin-1 jmin-1],[1 nd id jd]);
            data=nc_varget(fname,'water_temp',[ndays 0 imin-1 jmin-1],[1 nd id jd]);
            if ndims(data)==3
...
%            data=nc_varget(fname,'ssh',[ndays imin-1 jmin-1],[1 id jd]);
            data=nc_varget(fname,'surf_el',[ndays imin-1 jmin-1],[1 id jd]);
            if ndims(data)==2
                data(:,:,1)=data;
            else
                data=permute(data,[2 3 1]);
...
%            data=nc_varget(fname,'u',[ndays 0 imin-1 jmin-1],[1 nd id jd]);
            data=nc_varget(fname,'water_u',[ndays 0 imin-1 jmin-1],[1 nd id jd]);
            if ndims(data)==3
                data=permute(data,[2 3 1]);
                data(:,:,:,1)=data;
            else
...
%            data=nc_varget(fname,'v',[ndays 0 imin-1 jmin-1],[1 nd id jd]);
            data=nc_varget(fname,'water_v',[ndays 0 imin-1 jmin-1],[1 nd id jd]);
            if ndims(data)==3
                data=permute(data,[2 3 1]);
                data(:,:,:,1)=data;
            else
