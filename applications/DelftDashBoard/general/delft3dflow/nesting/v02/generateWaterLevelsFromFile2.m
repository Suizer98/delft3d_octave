function [times,wl] = generateWaterLevelsFromFile2(flow, openBoundaries, opt)
...
% $Date: 2015-07-16 09:50:03 +0200 (Thu, 16 Jul 2015) $
% $Author: ormondt $
% $Revision: 12105 $
    ...
    case{'dflowfm'}
        ...
        for i=1:nr
            for ip=1:length(openBoundaries(i).nodes)
                n=n+1;
                x(n,1)=openBoundaries(i).nodes(ip).x;
                y(n,1)=openBoundaries(i).nodes(ip).y;
            end
        end
end
...
switch lower(flow.model)
    ...
    case{'dflowfm'}
        ...
        for i=1:nr
            for ip=1:length(openBoundaries(i).nodes)
                n=n+1;
                wl(n,1,:) = spline(times,squeeze(wl0(n,1,:)),t);
            end            
        end
end
