...
[x1,y1]=convertCoordinates(x,y,'persistent','CS1.name',cs.name,'CS1.type',cs.type,'CS2.code',5070);
...
% Read NLCD values 
[nlcd,xa,ya,I] = ddb_geoimread(filename,[xmin xmax],[ymin ymax]);

xa=xa+15; % cell centres
ya=ya+15; % cell centres

if get_nlcd
    s.nlcd=interp2(xa,ya,nlcd,x1,y1,'nearest'); % Interpolate to grid
end
if get_manning
    manning=nlcd2manning(nlcd);                  % Convert NLCD to Manning
