...
%			If OPT == 'url2image' returns coordinates in geogs instead of Mercator.
...
%
	...
	switch lower(opt)
		...
		otherwise
			disp('Valid keewords are: ''tile2url'' ''tile2img'' ''quadcoord'' ''mappcache'' ')
	...
	try
		...
		if (~isempty(cache))		% We have a cache dir to look for
			...
			if (exist(fname, 'file'))
				...
				[img, cmap] = imread(url);
				saveInCache([cache cache_supp], fname, cmap, img, ext)	% Save tile in the cache directory (if CACHE exists)
			end
...
% --------------------------------------------------------------------------------------	
function saveInCache(cache, fname, cmap, img, ext)
...
% CMAP is the image colormap (when it applyies)
