function [netStruc,circ]=curv2net_v3(xg,yg,z)
...
for ii=1:ni
    for jj=1:nj
        if ~isnan(xg(ii,jj))
            nnodes=nnodes+1;
            netStruc.node.mesh2d_node_x(1,nnodes)=xg(ii,jj);
            netStruc.node.mesh2d_node_y(1,nnodes)=yg(ii,jj);
            netStruc.node.mesh2d_node_z(1,nnodes)=z(ii,jj);
            nodenr(ii,jj)=nnodes;
        else
            nodenr(ii,jj)=NaN;
        end
...
%                netStruc.edge.NetLink(2,nlinks)=nodenr(ii,jj+1);
                netStruc.edge.mesh2d_edge_nodes(1,nlinks)=nodenr(ii,jj);
                netStruc.edge.mesh2d_edge_nodes(2,nlinks)=nodenr(ii,jj+1);
%                linkType(nlinks,1)=2;
            end
        end
        if ii<size(xg,1)
...
%                netStruc.edge.NetLink(2,nlinks)=nodenr(ii+1,jj);
                netStruc.edge.mesh2d_edge_nodes(1,nlinks)=nodenr(ii,jj);
                netStruc.edge.mesh2d_edge_nodes(2,nlinks)=nodenr(ii+1,jj);
%                linkType(nlinks,1)=2;
            end
        end
    end
...
%             netStruc.face.FlowElemCont_y(:,nelems)=[yg(ii,jj);yg(ii+1,jj);yg(ii+1,jj+1);yg(ii,jj+1);NaN;NaN];
            netStruc.face.mesh2d_face_nodes(:,nelems)=[nodenr(ii,jj);nodenr(ii+1,jj);nodenr(ii+1,jj+1);nodenr(ii,jj+1);NaN;NaN];
        end
    end
end
...
[netStruc.tri,netStruc.map3,netStruc.ntyp] = patch2tri(netStruc.node.mesh2d_node_x,netStruc.node.mesh2d_node_y,netStruc.face.mesh2d_face_nodes,'quiet',1);
