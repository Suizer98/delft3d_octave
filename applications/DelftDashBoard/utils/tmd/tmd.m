% Tidal Model Driver (TMD) num2str
...
fprintf('1. Model and grid files should be in OTIS binary format,\n');
fprintf('   see http://www.oce.orst.edu/po/research/tide/inv_doc.html\n');
...
fprintf('5. Default model files location is subdir DATA. For each model\n');
fprintf('   a control file ''DATA/Model_*'' should be given. The file\n');
fprintf('   MUST contain 3 lines:\n');
...
fprintf('   lat,lon to x,y and back should be given in 4-th line, for example:\n');
fprintf('         ''xy_ll'' for Arctic or ''xy_ll_S'' for Antarctic\n');      
fprintf('\n');
%
[cfile,pname,findex]=uigetfile('Model_*','Open MODEL files listed in ');
CFname=[pname cfile];
fid=fopen(CFname,'r');
cfile=fgetl(fid);hfile=[ pname cfile];
cfile=fgetl(fid);ufile=[ pname cfile];
cfile=fgetl(fid);Gname=[ pname cfile];
Fxy_ll=fgetl(fid);
fclose(fid);
...
Mname=findMname(hfile,ufile);
tmpFileName=['tmp' Mname(5:end) '.mat'];
k=findstr(CFname,'/');
...
fprintf('Type ''help  extract_HC'',''help tide_pred'', ''help ellipse'',\n');
fprintf('Type ''help  get_coeff'',''help get_ellipse'',\n');
...
fprintf('Input file examples are in: lat_lon, lat_lon_1, lat_lon_2\n');
fprintf('\n');
fprintf('Programmed by: Lana Erofeeva, 2003\n');
fprintf('\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
CbkI=['vi=get(uinF,''Value'');if vi==1,set(uTinF,''Enable'',''on'');',...
      ...
      'set(uPlon,''Enable'',''off'');timeOnOff(uTime,uT,''off'');',...
      'else set(uTinF,''Enable'',''off'');',...      
      'set(uP,''Enable'',''on'');set(uPlat,''Enable'',''on'');',...
      'if mode==2,timeOnOff(uTime,uT,''on'');end;',...
...
uTinF=uicontrol('Style','Edit','units','normalized','position',pos5,...
         ...
         'Type ''help InFileFormat'' on Input File Format');
...
pos0=[p3(1)-0.01 p3(2)-2*bh-0.04 ax+0.01 2*bh+0.03];
...
Cbk5=['mode=get(uAT,''value'');vi=get(uinF,''Value'');',...
  'if mode==1, timeOnOff(uTime,uT,''off'');set(upar(1),''Enable'',''on'');',...
  'elseif vi==0, timeOnOff(uTime,uT,''on'');end;',...
...
uAT=uicontrol('Style','popupmenu','units','normalized','position',pos,...
             'string',ATs,'FontWeight','bold','FontSize',12,...
...
uPl=uicontrol('Style','popupmenu','units','normalized','position',pos,...
             'string',sPl1,'FontWeight','bold','FontSize',12,...
...
uPlat=uicontrol('Style','Edit','units','normalized','position',pos,...
       ...
       'clat=get(gcbo,''String'');lat=check_lat_lon(clat,1,uPlat);');
pos(1)=pos(1)+pos(3);
uPlon=uicontrol('Style','Edit','units','normalized','position',pos,...
       ...
       'clon=get(gcbo,''String'');lon=check_lat_lon(clon,2,uPlon);');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  RESTART and GO
pos=pos0;
...
Cbk4=['yy=str2num(get(gcbo,''String''));yy=check_date(yy,''yy'',uT(1));';...
      'mm=str2num(get(gcbo,''String''));mm=check_date(mm,''mm'',uT(2));';...
      'dd=str2num(get(gcbo,''String''));dd=check_date(dd,''dd'',uT(3));';...
      'hh=str2num(get(gcbo,''String''));hh=check_date(hh,''hh'',uT(4));';...
      'mi=str2num(get(gcbo,''String''));mi=check_date(mi,''mi'',uT(5));';...
      'nh=str2num(get(gcbo,''String''));nh=check_date(nh,''nh'',uT(6));';];
