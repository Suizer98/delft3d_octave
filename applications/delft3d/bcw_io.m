function Info=bcw_io(cmd,varargin)
%BCT_IO Read/write boundary condition tables.
%   INFO = BCW_IO('READ',FILENAME) opens the specified file and read all
...
%   particular file.
%
%   BCW_IO('WRITE',FILENAME,INFO) writes the time-series data to the
%   specified file. The INFO structure should be similar to the one
%   obtained from a BCT_IO('OPEN',...) call.
%
%   DATE = BCW_IO('TIMES',INFO,I) returns a vector DATE containing the
%   dates associated with table I of file INFO.
%
%   DATE = BCW_IO('TIMES',INFO,I,IDX) returns DATE(IDX) rather than the
%   full date vector.
%
%   INFO2 = BCW_IO('CLIP',INFO,T1,T2) clips all time series such that the
...
%   $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/delft3d/bcw_io.m $
%   $Id: bcw_io.m 16009 2019-11-18 21:34:09Z nederhof $
