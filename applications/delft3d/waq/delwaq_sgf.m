function Out = delwaq_sgf(cmd,varargin)
...
%
%   Struct = DELWAQ_SGF('write','FileName',Data,Time)
...
function S = delwaq_res_read(FileName,NSegments, T0)
...
if ischar(NSegments) && exist(NSegments,'file')
   Subs = read_subsName(NSegments);
   NSegments = length(Subs);
else
%     for i = 1:1
    Subs{1} = ['Sub' num2str(1, '%03.0f')]; 
%     end
end

fseek(fid,0,1);
S.NBytesBlock = ftell(fid);
S.NTimes = ((S.NBytesBlock-4)/4)/(NSegments+1);
S.NumSegm = NSegments;
S.NumSubs = 1;
fseek(fid,0,-1);

for i = 1:S.NTimes+1;
    S.Time(i) = fread(fid,[1 1],'uint32');
    S.data(i,:) = fread(fid,[S.NumSubs S.NumSegm],'real*4');
end
...
if nargin == 2 && isstruct(varargin{2})
   ...
   [S.NTimes, S.NumSegm] =  size(S.data);
   S.NBytesBlock = (S.NTimes * S.NumSegm*4) + 4;
end
