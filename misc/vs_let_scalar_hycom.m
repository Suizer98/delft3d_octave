function scalar = vs_let_scalar_hycom(varargin)
...
case {'Delft3D-com','Delft3D-tram','Delft3D-botm','Delft3D-trim'},
   ...
   if dims(1)==1 
      scalar = permute(scalar(:,2:end-1,2:end-1,:,:),[3 2 4 5 1]); % do not use reshape, that goes wrong for a grid of just one row/columnelse
   else   
